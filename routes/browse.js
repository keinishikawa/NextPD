...
// 新規登録画面の表示
  router.get('/signup', (req, res) => {
    res.render('./components/browse/signup.ejs', { errors: [] });
  });
  ...
  router.post('/signup', 
    ...
    (req, res, next) => {
      ...
      if (errors.length > 0) {
        res.render('./components/browse/signup.ejs', { errors: errors });
      } else {
        next();
      }
    },
...
// ============トップ画面以降の処理=======================
  ...
  router.get("/friends-list",(req,res)=>{
    ...
    connection.query(
      ...
      [email],
      (error, results) => {
        res.render('./components/browse/friends-list.ejs', { friends: results });
      }
    );
  });
...
router.get("/matching-main",(req,res)=>{
  ...
  connection.query(
    ...
    [email],
    (error, results) => {
      res.render('./components/browse/matching-main.ejs', { freeFriends : results });
    }
  );
})
